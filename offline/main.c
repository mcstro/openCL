...
//        fprintf(stdout,"[%d]: %1.4f squared = %1.4f...\n",i,input[i],output[i]);
    }
...
//   Notes:
//   
// 1 Include the header file that contains the kernel block declaration. The name of the
//   header file for a .cl file will be the name of the .cl file with .h appended to it. 
//   For example, if the .cl file is named mykernel.cl, the header file you must include 
//   will be mykernel.cl.h.
//
// 2 Call gcl_create_dispatch_queue to create the dispatch queue.
//
// 3 Create memory objects to hold input and output data and write input data to the 
//   input objects. Allocate an array on the OpenCL device from which to read kernel
//   results back into host memory. Use gcl_malloc and make sure to use the OpenCL size 
//   of the datatype being returned. For example, write gcl_malloc(sizeof(cl_float) * NUM_VALUES. 
//   Because the CL device operates on its own memory space, allocate OpenCL memory for the 
//   input data upon which the kernel will work. Specify CL_MEM_COPY_HOST_PTR to tell OpenCL 
//   to copy over the input data from host memory into its memory space before it executes 
//   the kernel.
//
// 4 Allocate OpenCL memory in which the kernel will store its results.
//
// 5 Dispatch your kernel block using one of the dispatch commands and the queue you created
//   above. In your dispatch call, you can specify workgroup parameters.
//
// 6 Describe the _data parallel range_ (the ndrange) over which to execute the kernel in 
//   the cl_ndrange structure.
//
//   OpenCL always executes kernels in a data parallel fashion—that is, instances of the same 
//   kernel (work items) execute on different portions of the total data set. Each work item is 
//   responsible for executing the kernel once and operating on its assigned portion of the data set.
//   
//   You use the cl_ndrange field to specify how the workgroups are to be organized. For more 
//   information, see Specifying How To Divide Up A Dataset.
//   
// 7 Always pass an offset for each of three dimensions even though the workgroup may have fewer 
//   than three dimensions. See Specifying How To Divide Up A Dataset for more information.
//
// 8 Call the kernel as you would call a function. Pass the ndrange as the first parameter, 
//   followed by the expected kernel parameters. Case the void* types to the expected OpenCL 
//   types. Remember, if you use float in your kernel, that's a cl_float from the application's 
//   perspective. The call to the kernel will look something like this:
//
//   kernelName(
//          &ndrange,
...
//
// 9 Retrieve the data from the OpenCL device's memory space with gcl_memcpy. The output 
//   computed by the kernel is copied over to the host application's memory space.
